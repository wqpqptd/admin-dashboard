...
const LicenseDuration = () => {
  ...
  return (
    <>
      ...
      {/* <!-- ====== LicenseDuration Section Start ====== --> */}
      <div className="rounded-sm border border-stroke bg-white px-5 pt-6 pb-2.5 shadow-default dark:border-strokedark dark:bg-boxdark sm:px-7.5 xl:pb-1">
        <div className="max-w-full overflow-x-auto">
          <Link href={"/licenseManager/licenseDuration/create"}>
            <button className="flex w-auto justify-center rounded bg-primary p-3 font-medium text-gray">Thêm thời hạn giấy phép lái xe</button>
            <br />
          </Link>
              ...
              {packageItems.map((packageItem: any, index :number) => (
                <tr key={packageItem.id}>
                  ...
                  <td className="border-b border-[#eee] py-5 px-4 dark:border-strokedark">
                    <Link className="mb-5.5 inline-block hover:text-primary" href={`licenseManager/licenseDuration/${packageItem.id}`}>
                      {packageItem.duration}
                    </Link>
                  </td>
                  <td className="border-b border-[#eee] py-5 px-4 dark:border-strokedark">
                    <div className="flex items-center space-x-3.5">
                    <Link className="hover:text-primary" href={`/licenseManager/licenseDuration/update?id=${packageItem.id}`}>
                        <svg
