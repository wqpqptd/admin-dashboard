...
const LicenseClass = () => {
  ...
  const deleteLicense = (licenseId: any) => {
    axios.delete(`${URL_SERVER}/driverlicenseclass/${licenseId}`)
      .then(response => {
              ...
              {packageItems.map((packageItem: any, index :number) => (
                <tr key={packageItem.id}>
                  ...
                  <td className="border-b border-[#eee] py-5 px-4 dark:border-strokedark">
                    <Link className="mb-5.5 inline-block hover:text-primary" href={`licenseManager/licenseClass/${packageItem.id}`}>
                      {packageItem.driverLicenseClassName}
                    </Link>
                  </td>
                  <td className="border-b border-[#eee] py-5 px-4 dark:border-strokedark">
                    <div className="flex items-center space-x-3.5">
                      <Link className="hover:text-primary" href={`/licenseManager/licenseClass/update?id=${packageItem.id}`}>
                        <svg
                      ...
                      <button onClick={() => deleteLicense(packageItem?.id)} className="hover:text-primary">
